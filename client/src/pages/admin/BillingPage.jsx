...
const BillingPage = () => {
  ...
  const downloadInvoice = async (orderId) => {
    try {
      const response = await axiosInstance.get(`/admin/download-invoice/${orderId}`, {
        responseType: 'blob',  
      });
  ...
  const calculateCourseDistribution = (orders) => {
    ...
    orders.forEach((order) => {
      order.courses.forEach((course) => {
        ...
        if (categoryTitle) {
          revenuePerCategory[categoryTitle] = 
           parseInt ( (revenuePerCategory[categoryTitle] || 0) + (course.price?.$numberDecimal || 0) );
        }
      });
    });
 ...
 const totalRevenue= orders.filter((order)=> order.orderStatus==='Completed').reduce((acc,order)=> acc+ order.totalAmount,0);
 const totalCoursesSold = orders.filter((order)=> order.orderStatus==='Completed').length
    ...
    <div className='flex'>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-gray-500 text-sm font-medium">Total Courses Sold</h3>
            <p className="text-2xl font-bold text-gray-900">{totalCoursesSold}</p>
            <div className="flex items-center mt-2">
              <span className="text-green-500 text-sm flex items-center">
                <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 10l7-7m0 0l7 7m-7-7v18" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Course Sales Distribution</h3>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
       ...
       {/* Pagination */}
       <Pagination className='flex justify-center mb-12' dataPerPage={dataPerPage} setCurrentPage={setCurrentPage} currentPage={currentPage} totalData={orders.length}/>
             </div>
    </div>
    </div>
